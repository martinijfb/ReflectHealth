...
struct ScannerViewRepresentable: UIViewRepresentable {
    ...
    func updateUIView(_ uiView: ARView, context: Context) {
        ...
        if vm.shouldStartSession {
            let configuration = ARFaceTrackingConfiguration()
            uiView.session.run(configuration, options: [.resetTracking, .removeExistingAnchors])
            DispatchQueue.main.async {
                vm.shouldPauseSession = false
                vm.shouldStartSession = false
                vm.didInitiallyCalibrate = false
            }
        }
    ...
    func makeCoordinator() -> Coordinator {
        Coordinator(status: $vm.status, matrix: $vm.matrix, imageData: $vm.imageData, didInitiallyCalibrate: $vm.didInitiallyCalibrate)
    }
